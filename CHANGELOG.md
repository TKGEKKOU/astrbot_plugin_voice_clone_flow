...
- Windows FFmpeg 下载自动并行测速官方源与国内加速镜像（BtbN / Gyan / ghfast.top / gh-proxy.com），选择实测吞吐最快的源下载，无需手动挑选；用户显式配置的下载地址始终优先。
- Windows FFmpeg 下载支持断点续传：中断或切换源后保留断点文件，重试时基于 `Range` 继续下载，不再从零开始。
